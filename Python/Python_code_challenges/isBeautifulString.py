...
previous letter in the alphabet within the string; ie: b occurs no more times than a; c occurs 
...
Example
    ...
    This string contains 3 as, 3 bs, 1 c, 1 d, 1 e, and 1 f (and 0 of every other letter), so 
    since there aren't any letters that appear more frequently than the previous letter, this string 
    qualifies as beautiful.
...
def isBeautifulString(s):
    lst = [s.count(x) for x in 'abcdefghijklmnopqrstuvwxyz']
